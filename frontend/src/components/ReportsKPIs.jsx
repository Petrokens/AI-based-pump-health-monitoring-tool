    ...
    <div className="bg-[var(--bg-card)] rounded-xl p-6 border border-[var(--border-color)] mb-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-[var(--bg-secondary)]/50 rounded-lg p-4">
          ...
          <p className="text-xs text-[var(--text-secondary)] mt-1">Availability</p>
        </div>
        ...
        <div className="bg-[var(--bg-secondary)]/50 rounded-lg p-4">
          ...
          <p className="text-xs text-[var(--text-secondary)] mt-1">Target: {data.sla_target}%</p>
        </div>

        <div className="bg-[var(--bg-secondary)]/50 rounded-lg p-4">
          <div className="flex items-center mb-2">
            <Clock className="w-4 h-4 text-yellow-400 mr-2" />
            <span className="text-xs text-[var(--text-secondary)]">Interventions</span>
          </div>
          <p className="text-2xl font-bold text-[var(--text-primary)]">{data.number_of_interventions}</p>
          <p className="text-xs text-[var(--text-secondary)] mt-1">Maintenance events</p>
        </div>

        <div className="bg-[var(--bg-secondary)]/50 rounded-lg p-4">
          <div className="flex items-center mb-2">
            <DollarSign className="w-4 h-4 text-green-400 mr-2" />
            <span className="text-xs text-[var(--text-secondary)]">Cost Avoided</span>
          </div>
          <p className="text-2xl font-bold text-green-400">${data.cost_avoided.toLocaleString()}</p>
          <p className="text-xs text-[var(--text-secondary)] mt-1">Estimated savings</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-[var(--bg-secondary)]/50 rounded-lg p-4">
          <span className="text-xs text-[var(--text-secondary)]">Total Downtime</span>
          <p className="text-xl font-bold text-[var(--text-primary)]">{data.total_downtime_hours.toFixed(1)}h</p>
          <p className="text-xs text-[var(--text-secondary)] mt-1">Report period</p>
        </div>

        <div className="bg-[var(--bg-secondary)]/50 rounded-lg p-4">
          <span className="text-xs text-[var(--text-secondary)]">Downtime Cost</span>
          <p className="text-xl font-bold text-red-400">${data.downtime_cost.toLocaleString()}</p>
          <p className="text-xs text-[var(--text-secondary)] mt-1">Estimated cost</p>
        </div>

        <div className="bg-[var(--bg-secondary)]/50 rounded-lg p-4">
          <span className="text-xs text-[var(--text-secondary)]">Report Period</span>
          <p className="text-sm font-semibold text-[var(--text-primary)]">
            {data.report_period.start} to {data.report_period.end}
          </p>
          <p className="text-xs text-[var(--text-secondary)] mt-1">Last 28 days</p>
        </div>
      </div>
      ...
      {data.weekly_reports && data.weekly_reports.length > 0 && (
        <div className="mb-6">
          <h3 className="text-sm font-semibold text-[var(--text-secondary)] mb-3">Weekly Performance</h3>
      ...
      {data.weekly_reports && data.weekly_reports.length > 0 && (
        <div>
          <h3 className="text-sm font-semibold text-[var(--text-secondary)] mb-3">Uptime Trend</h3>
          <div className="bg-[var(--bg-secondary)]/30 rounded-lg p-4">
      ...
      {/* Summary Table */}
      <div className="mt-6">
        <h3 className="text-sm font-semibold text-[var(--text-secondary)] mb-3">Weekly Summary</h3>
        <div className="bg-[var(--bg-secondary)]/30 rounded-lg overflow-hidden">
          <table className="w-full">
            <thead className="bg-[var(--bg-tertiary)]/50">
              <tr>
                <th className="text-left p-3 text-xs text-[var(--text-secondary)]">Week</th>
                <th className="text-right p-3 text-xs text-[var(--text-secondary)]">Interventions</th>
                <th className="text-right p-3 text-xs text-[var(--text-secondary)]">Downtime (h)</th>
                <th className="text-right p-3 text-xs text-[var(--text-secondary)]">Uptime %</th>
              </tr>
            </thead>
            <tbody>
              {data.weekly_reports.map((week, idx) => (
                <tr key={idx} className="border-t border-[var(--border-light)]">
                  <td className="p-3 text-sm text-[var(--text-primary)]">{week.week}</td>
                  <td className="p-3 text-sm text-[var(--text-primary)] text-right">{week.interventions}</td>
                  <td className="p-3 text-sm text-[var(--text-primary)] text-right">{week.downtime_hours.toFixed(1)}</td>
